...
## [5.0.3](https://github.com/ckeditor/ckeditor5-inspector/compare/v5.0.2...v5.0.3) (July 14, 2026)

### Bug fixes

* Fixed the `container` option of `CKEditorInspector.attach()` to actually work in multi-window / multi-document environments (Electron, WebView2, iframes). Previously, even when a container in a different document was passed, the inspector's UI still read and wrote the module-realm `document` and `window` (event listeners, `body` class toggles, viewport sizing, `Modal` app element, `scrollIntoView` targets), so it appeared broken in the host document. Closes [#39](https://github.com/ckeditor/ckeditor5-inspector/issues/39), [#100](https://github.com/ckeditor/ckeditor5-inspector/issues/100).
